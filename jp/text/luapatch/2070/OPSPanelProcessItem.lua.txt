...
local RefreshUI = function(self)
	...
	if self.info.diffcluty == -1 then
		...
		if CS.GameData.missionAction ~= nil and CS.GameData.missionAction.mission == self.info.mission then
			self.transform:Find("Mission/MissionInProgress").gameObject:SetActive(true);
		else
			self.transform:Find("Mission/MissionInProgress").gameObject:SetActive(false);
		end
		return;
	end
	self:RefreshUI();
