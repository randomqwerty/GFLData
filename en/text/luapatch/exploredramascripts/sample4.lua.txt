...
Init = function()
    ...
    local ai416;
    local aiG11;
    local battle = math.random()>=0.5;
    for i=1,5 do
        if i-1 < dramaUtil.GetTeam().Count then
            ai[i] = dramaUtil.CreateAIWithGun(dramaUtil.GetTeam()[i-1],v2int(i+4,i*4+10),false);
            if dramaUtil.GetTeam()[i-1].info.code == 'G11' then
                dramaUtil.PlanAIAppear(ai[i],0,dramaUtil.Direction.right,"lying");
                dramaUtil.PlanAIStay(ai[i],10,dramaUtil.Direction.dontChange,"lying");
            else
