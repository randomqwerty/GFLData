...
Awake = function()
	...
	local SetFariy = function(self,fairy)
		self:SetFariy(CS.Fairy(CS.Data.GetFakeFairyInfo()))
	end
	...
	util.hotfix_ex(CS.BattleFriendlyCharacterController,'Init',Init)
...
InitCharacter = function()
	...
	enemyTransform =  BattleController.enemyTeamHolder:GetCharacters()[0].transform
	...
	enemyStartingPos = BattleController.enemyTeamHolder:GetCharacters()[0].transform.localPosition
end
...
Update = function()
	...
	if character == nil then
		character = CS.BattleLuaUtility.GetCharacterByCode(gunCode)
		if character == nil then	
			return
		else
			InitCharacter()
	...
	if targetBuff == nil then
		...
		for i=0,character.conditionListSelf.mListCondition.Count-1 do
			if character.conditionListSelf.mListCondition[i].id == layerBuffID then
				targetBuff = character.conditionListSelf.mListCondition[i]
				break
			end
		end
	end
	...
	if character.conditionListSelf:GetTierByID(layerBuffID) > 0 then
		local t = (tempvalue + character.conditionListSelf:GetTierByID(layerBuffID)) / 1000
	...
	if (not showDialogue) and character.conditionListSelf:GetTierByID(layerBuffID) >= endingBuffTier then
		showDialogue = true
		CS.BattleLuaUtility.CharaacterPlaySkill(11981401,character)
...
OnDestroy =function()
	...
	xlua.hotfix(CS.BattleFriendlyCharacterController,'Init',nil)
	xlua.hotfix(CS.BattleInteractionController,'DisableRangeLine',nil)
	xlua.hotfix(CS.GF.Battle.BattleFairyController,'SetFariy',nil)
end
